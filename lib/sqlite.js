...
async function executeQuery(sql, params = [], method = 'all') {
    if (!connectionString) {
        // Real-time check to see what's happening on Vercel
        console.error('🕵️ [DEBUG_REQ] connectionString is FALSEY inside request');
        console.error('🕵️ [DEBUG_REQ] process.env.SQLITECLOUD_CONNECTION_STRING:',
            process.env.SQLITECLOUD_CONNECTION_STRING ? 'DEFINED_BUT_HIDDEN' : 'UNDEFINED_OR_EMPTY');
        const keys = Object.keys(process.env).filter(k => k.includes('SQL') || k.includes('DB'));
        console.error('🕵️ [DEBUG_REQ] Available relevant env keys:', keys.join(', '));
    }
...
async function getFondos(ids, paths = null) {
    ...
    if (!ids || ids.length === 0) {
        const funds = await executeQuery('SELECT id, name, fund_id, min_investment, currency_id, tipo_renta_id, fondo_horizonte_vieja, estado, return_day, return_month, aum FROM funds');
        ...
        return funds.map(f => ({
            ...
            rendimientoDia: f.return_day,
            rendimientoMes: f.return_month,
            patrimonio: f.aum
        }));
    }
