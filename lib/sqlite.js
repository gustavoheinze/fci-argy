...
if (connectionString) {
    ...
    try {
        // Dynamic require to avoid Vercel build errors with persistent native modules
        const BetterSqlite3 = require('better-sqlite3');
        db = new BetterSqlite3(DB_PATH);
        if (db.pragma) db.pragma('journal_mode = WAL');
    } catch (err) {
        console.warn('⚠️ [DB_LOCAL_WARNING] better-sqlite3 not found/loadable. Local database unavailable.');
        if (!connectionString) {
            console.error('❌ [DB_FATAL_ERROR] No database configured! Set SQLITECLOUD_CONNECTION_STRING.');
        }
    }
}
